#!/usr/bin/env python3
"""Compile and run the ASCII (keyboard-only) torus. Ctrl+C to bail."""
...
C_SRC = os.path.join(HERE, "donut_latin.c")


def bin_path(src: bytes) -> str:
    digest = hashlib.sha256(src).hexdigest()[:16]
    return os.path.join(tempfile.gettempdir(), f"torus_latin_{digest}")
