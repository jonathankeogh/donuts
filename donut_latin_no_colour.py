#!/usr/bin/env python3
"""Compile and run the monochrome ASCII torus. Ctrl+C to bail."""
...
C_SRC = os.path.join(HERE, "donut_latin_no_colour.c")


def bin_path(src: bytes) -> str:
    digest = hashlib.sha256(src).hexdigest()[:16]
    return os.path.join(tempfile.gettempdir(), f"torus_latin_nc_{digest}")
